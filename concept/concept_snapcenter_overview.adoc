...
** 位于ONTAP FAS或AFF主系统和辅助系统上的数据，并使用NetApp备份和恢复集成保护到本地StorageGRID对象存储


* 内部部署在混合云中以保护以下内容：
+
** 位于ONTAP FAS 或AFF 主系统上并复制到Cloud Volumes ONTAP 的数据
** 位于ONTAP FAS或AFF主系统和辅助系统上的数据，使用NetApp备份和恢复集成保护到云中的对象和归档存储
...
+
有关Windows cmdlet的详细信息，请参阅 https://docs.netapp.com/us-en/snapcenter-cmdlets-48/index.html["《 SnapCenter 软件 cmdlet 参考指南》"^]。
